...
void kernel(float* data, const float alpha, const int N) {
    ...
    while (true) {
        ...
        tma::cluster::wait(clc_arrived, get_phasebit<0>(phasebit, 0));
        ...
        curr_block_idx = next_cta_id.x + ctarank; // must add cluster-local block index
