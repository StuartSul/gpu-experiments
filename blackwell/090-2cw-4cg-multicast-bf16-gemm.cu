...
static constexpr int CLUSTER_N = 2*Nb;
...
using a_tile = st_bf<Mb, Kb, true, 128>;
using a_subtile = st_bf<Mb, Kb/2, true, 128>;
...
__global__ void kernel(const __grid_constant__ globals g) {
    ...
    const int cta_x = cta_rank&1;
    const int cta_y = (cta_rank&2)>>1;
    ...
    if (threadIdx.x == 0) { 
        ...
        for(int i = 0; i < PIPE_DEPTH; i++) {
            ...
            init_semaphore(inputs_finished[i], 0, CLUSTER_SIZE);
        }
        init_semaphore(outputs_arrived, 0, 1);
        #pragma unroll
        for(int i = 0; i < NUM_CONSUMERS; i++) {
            init_semaphore(outputs_finished[i], 0, NUM_CONSUMERS);
        }
    }
    everyone::tma::cluster::sync();
    ...
    if(warpgroupid == NUM_CONSUMERS) {
        ...
        if(warp::laneid() == 0 && warpgroup::warpid() == 3) {
            ...
            for(int task_iter = 0; true; task_iter++) {
                ...
                for (int idx = 0; idx < iters_per_task; idx++) {
                    ...
                    tma::cluster::load_async(a_smem[input_ring][0].subtile<Kb/2>(cta_y), g.a, {rowcol.x*4+cta_x*2+0, idx*2+cta_y}, inputs_arrived[input_ring], (uint16_t)(0b101<<cta_x), cta_rank&2);
                    tma::cluster::load_async(a_smem[input_ring][1].subtile<Kb/2>(cta_y), g.a, {rowcol.x*4+cta_x*2+1, idx*2+cta_y}, inputs_arrived[input_ring], (uint16_t)(0b101<<cta_x), cta_rank&2);
                    tma::cluster::load_async(b_smem[input_ring],        g.b, {rowcol.y*4+cta_rank,      idx}, inputs_arrived[input_ring], (uint16_t)(1<<cta_rank),  cta_rank&2);
                    input_ring=ring_advance<PIPE_DEPTH>(input_ring);
                }
            }
        }
        else if(cta_rank%2 == 0 && warp::laneid() == 0 && (warpgroup::warpid() == 0 || warpgroup::warpid() == 1)) { // launch the MMA's
            ...
            for(int task_iter = 0; true; task_iter++) {
                ...
                tma::cluster::expect_bytes(inputs_arrived[input_ring], 2*sizeof(a_tile) + sizeof(b_tile));
                tma::cluster::wait(inputs_arrived[input_ring], get_phasebit<0>(bitfield, input_ring));
                update_phasebit<0>(bitfield, input_ring);
                mm2_ABt(d_tt, a_smem[input_ring][warpgroup::warpid()], b_smem[input_ring]);
                detail::tcgen05::commit<2>(inputs_finished[input_ring], 0b1111);
                input_ring=ring_advance<PIPE_DEPTH>(input_ring);
        ...
        for(int task_iter = 0; true; task_iter++) {
            ...
            if(warpgroup::laneid() == 0) kittens::warp::tma::cluster::arrive(outputs_finished[warpgroupid], cta_rank&2); // Tensor memory for warpgroup 0 is now free.
            if(warpgroupid == 0) group<8>::sync(15);
            if(warpgroupid == 1) group<8>::sync(14);
            warpgroup::store(d_smem, d_reg[0]);
            warpgroup::sync(warpgroupid);
            warpgroup::tma::store_async(g.d, d_smem, {4*rowcol.x+2*cta_x+warpgroupid, 4*(2*rowcol.y+cta_y)+0});
            #pragma unroll
            for(int i = 1; i < Nb/d_tile::cols; i++) {
                ...
                warpgroup::sync(warpgroupid);
                warpgroup::tma::store_async(g.d, d_smem, {4*rowcol.x+2*cta_x+warpgroupid, 4*(2*rowcol.y+cta_y)+i});
            }
            tma::store_async_read_wait();
