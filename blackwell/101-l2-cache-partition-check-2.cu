// Not much learnging here either
#include <random>
#include <vector>
...
template <int _N, int _REPEAT, int _TILE_N>
struct globals {
...
__global__ void kernel(const __grid_constant__ globals_t g) {
    ...
    if (sm_id != g.sm_id_0 && sm_id != g.sm_id_1) return;
...
template <typename globals_t>
__host__ void inner_run(int sm_id_0, int sm_id_1) {
    constexpr int N = globals_t::N;
    constexpr int REPEAT = globals_t::REPEAT;
    constexpr int SIZE = N * N * sizeof(bf16);

    printf("SM_ID_0=%03d, SM_ID_1=%03d, N=%d, REPEAT=%d, Size: %.3f MB, ", sm_id_0, sm_id_1, N, REPEAT, SIZE / 1024. / 1024.);
    ...
    std::mt19937 gen(42 + 148 * sm_id_0 + sm_id_1);
    ...
    globals_t g{a_gl, sm_id_0, sm_id_1};
    ...
    cudaFree(reinterpret_cast<void *>(base_addr));
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
}

template <int N, int REPEAT, int TILE_N>
__host__ void outer_run() {
    for (int sm_id_0 = 0; sm_id_0 < 148; sm_id_0++) {
        for (int sm_id_1 = sm_id_0 + 1; sm_id_1 < 148; sm_id_1++) {
            inner_run<globals<N, REPEAT, TILE_N>>(sm_id_0, sm_id_1);
        }
    }
}

__host__ int main() {
    outer_run<2048, 128, 128>();
}
