/*
    ...
    "Pull" variant of 001: the kernel runs on the DESTINATION device, which pulls data from the
    source device. Data flows: remote src HBM -> NVLink -> local smem -> local dst HBM.
    (001 is the "push" variant: kernel on the source device writes out over NVLink.)
    All GB300, driven from the SMs using ThunderKittens TMA.

    GPU N -> GPU 0 pull ablation (10 GB), kernel on GPU 0:
        1 -> 0: 738.04 GB/s
        2 -> 0: 738.01 GB/s
        3 -> 0: 738.05 GB/s
*/
...
double benchmark(int srcDev, int dstDev) {
    ...
    printf("NVLink Bandwidth Test (TMA-based Pull Copy)\n");
    printf("============================================\n");
    ...
    printf("Direction: Device %d -> Device %d (pulled by Device %d)\n\n", srcDev, dstDev, dstDev);
    ...
    // Create events for timing on the destination device (where the pull kernel runs)
    CUDACHECK(cudaSetDevice(dstDev));
    ...
    // Warm up run (kernel launches on the destination device, which pulls the data)
    printf("Performing warm-up TMA pull transfer (puller Device %d)...\n", dstDev);
    CUDACHECK(cudaSetDevice(dstDev));
    ...
    // Timed TMA transfer
    printf("\nStarting timed TMA pull transfer: Device %d -> Device %d (puller Device %d)\n", srcDev, dstDev, dstDev);
    CUDACHECK(cudaSetDevice(dstDev));
...
int main() {
    // Ablation: TMA pull bandwidth into GPU 0 from each peer GPU (kernel runs on GPU 0).
    double bw10 = benchmark(1, 0);
    double bw20 = benchmark(2, 0);
    double bw30 = benchmark(3, 0);
    
    printf("==================== Ablation Summary ====================\n");
    printf("GPU 1 -> GPU 0 (pull): %.2f GB/s\n", bw10);
    printf("GPU 2 -> GPU 0 (pull): %.2f GB/s\n", bw20);
    printf("GPU 3 -> GPU 0 (pull): %.2f GB/s\n", bw30);
