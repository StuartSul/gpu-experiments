/*
    ...
    Device srcDev -> Device dstDev, all GB300, driven from the SMs using ThunderKittens TMA.

    GPU0 -> GPUN TMA ablation (10 GB):
        0 -> 1: 669.24 GB/s
        0 -> 2: 669.28 GB/s
        0 -> 3: 669.28 GB/s
*/
...
double benchmark(int srcDev, int dstDev) {
    ...
    // Timed TMA transfer
    printf("\nStarting timed TMA transfer: Device %d -> Device %d\n", srcDev, dstDev);
    CUDACHECK(cudaSetDevice(srcDev));
...
int main() {
    // Ablation: TMA copy bandwidth from GPU 0 to each peer GPU.
