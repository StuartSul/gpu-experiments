...
using mlp_bf16_t_tile = st_bf<config::MLP_BF16_Kb, config::MLP_Mb / 2>; // shared-expert (BF16) wgrad
...
static __device__ __forceinline__ void dispatch_quantize_kernel(
    ...
    const int cols = x_fp8_gmem.cols();
    ...
    for (int subtile = 0; subtile < config::DISPATCH_Nb / config::QUANT_Nb; ++subtile) {
        if (subtile < num_subtiles) {
            ...
            if (tid == 0) {
                ...
                tma::store_async(x_fp8_gmem, x_fp8_tiles[out], {row_block, col_block});
                tma::store_async(x_sc_gmem, x_sc_tiles[out], {row_block, col_block, 0, 0});
                tma::store_async(x_fp8_t_gmem, x_fp8_t_tiles[out], {col_block, row_block});
...
static __device__ __forceinline__ void swiglu_fwd(
    ...
    using gate_up_tile = std::conditional_t<IS_SHARED, swiglu_tile, quant_fp8_tile>;

    auto (&gate_smem)[config::SWIGLU_FWD_PIPE_DEPTH]    = *reinterpret_cast<gate_up_tile (*)[config::SWIGLU_FWD_PIPE_DEPTH]>(smem_base_addr);
    auto (&up_smem)[config::SWIGLU_FWD_PIPE_DEPTH]      = *reinterpret_cast<gate_up_tile (*)[config::SWIGLU_FWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&gate_smem) + sizeof(gate_smem));
    auto &hidden_bf16_smem                              = *reinterpret_cast<quant_bf16_tile *>(reinterpret_cast<uint64_t>(&up_smem) + sizeof(up_smem));
    auto (&gate_sc_smem)[config::SWIGLU_FWD_PIPE_DEPTH] = *reinterpret_cast<quant_sc_tile (*)[config::SWIGLU_FWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&hidden_bf16_smem) + sizeof(hidden_bf16_smem));
    auto (&up_sc_smem)[config::SWIGLU_FWD_PIPE_DEPTH]   = *reinterpret_cast<quant_sc_tile (*)[config::SWIGLU_FWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&gate_sc_smem) + sizeof(gate_sc_smem));
        ...
        if (tile_idx < tile_end) {
            ...
            if constexpr (!IS_SHARED) {
                ...
                if (threadIdx.x == 0) {
                    ...
                    tma::store_async(*hidden_sc_gmem, gate_sc_smem[stage], {row - macrobatch_row_block_offset, col, 0, 0});
                    tma::store_async(*hidden_fp8_t_gmem, up_smem[stage], {col, row - macrobatch_row_block_offset});
                    tma::store_async(*hidden_sc_t_gmem, up_sc_smem[stage], {col, row - macrobatch_row_block_offset, 0, 0});
                }
...
static __device__ __forceinline__ void swiglu_bwd(
    ...
    using gate_up_tile = std::conditional_t<IS_SHARED, swiglu_tile, quant_fp8_tile>;
    using d_hidden_tile = std::conditional_t<IS_SHARED, swiglu_tile, quant_bf16_tile>;

    auto (&d_hidden_smem)[config::SWIGLU_BWD_PIPE_DEPTH] = *reinterpret_cast<d_hidden_tile (*)[config::SWIGLU_BWD_PIPE_DEPTH]>(smem_base_addr);
    auto (&gate_smem)[config::SWIGLU_BWD_PIPE_DEPTH]     = *reinterpret_cast<gate_up_tile (*)[config::SWIGLU_BWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&d_hidden_smem) + sizeof(d_hidden_smem));
    auto (&up_smem)[config::SWIGLU_BWD_PIPE_DEPTH]       = *reinterpret_cast<gate_up_tile (*)[config::SWIGLU_BWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&gate_smem) + sizeof(gate_smem));
    auto (&gate_sc_smem)[config::SWIGLU_BWD_PIPE_DEPTH]  = *reinterpret_cast<quant_sc_tile (*)[config::SWIGLU_BWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&up_smem) + sizeof(up_smem));
    auto (&up_sc_smem)[config::SWIGLU_BWD_PIPE_DEPTH]    = *reinterpret_cast<quant_sc_tile (*)[config::SWIGLU_BWD_PIPE_DEPTH]>(reinterpret_cast<uint64_t>(&gate_sc_smem) + sizeof(gate_sc_smem));
    ...
    auto (&d_t_fp8_smem)[2]                              = *reinterpret_cast<quant_fp8_tile (*)[2]>(reinterpret_cast<uint64_t>(&d_up_bf16_smem) + sizeof(d_up_bf16_smem));
    auto (&d_t_sc_smem)[2]                               = *reinterpret_cast<quant_sc_tile (*)[2]>(reinterpret_cast<uint64_t>(&d_t_fp8_smem) + sizeof(d_t_fp8_smem));
            ...
            if constexpr (!IS_SHARED) {
                ...
                for (int j = 0; j < 2; ++j) { // 64 elements per thread = 2 32-element blocks per thread
                    ...
                    for (int k = 0; k < 16; ++k) {
                        ...
                        d_gate_bf16[k] = __float22bfloat162_rn(float2{dsilu_x * up_fp32[k].x * d_hidden_fp32[k].x,
                                                                      dsilu_y * up_fp32[k].y * d_hidden_fp32[k].y});
                        d_up_bf16[k] = __float22bfloat162_rn(float2{silu_x * d_hidden_fp32[k].x,
                                                                    silu_y * d_hidden_fp32[k].y});
                    }
                ...
                if (threadIdx.x == 0) {
                    tma::store_async(*d_gate_fp8_t_gmem, d_t_fp8_smem[0], {col, row - macrobatch_row_block_offset});
                    tma::store_async(*d_gate_sc_t_gmem, d_t_sc_smem[0], {col, row - macrobatch_row_block_offset, 0, 0});
                    tma::store_async(*d_up_fp8_t_gmem, d_t_fp8_smem[1], {col, row - macrobatch_row_block_offset});
...
static __device__ __forceinline__ void expert_grouped_gemm(
    ...
    using ab_tile = std::conditional_t<IS_SHARED, std::conditional_t<IS_WGRAD, mlp_bf16_t_tile, mlp_bf16_tile>, mlp_fp8_tile>;
    constexpr int MLP_Kb = IS_SHARED ? config::MLP_BF16_Kb : config::MLP_FP8_Kb;
    ...
    auto (&d_fp8_smem)[config::MLP_NUM_FP8_D_TILES]   = *reinterpret_cast<mlp_fp8_d_tile (*)[config::MLP_NUM_FP8_D_TILES]>(&d_bf16_smem[0]);
    auto (&d_sc_smem)[2]                              = *reinterpret_cast<mlp_sc_tile (*)[2]>(reinterpret_cast<uint64_t>(&d_fp8_smem) + sizeof(d_fp8_smem));

    const int col_blocks = (IS_WGRAD && IS_SHARED) ? b_gmem.cols() / config::MLP_Nb : b_gmem.rows() / config::MLP_Nb;
    const int global_minibatch_idx = macrobatch_idx * (macrobatch_size / minibatch_size) + minibatch_idx;
    ...
    if constexpr (IS_WGRAD) {
        const int row_blocks = IS_SHARED ? a_gmem.cols() / config::MLP_Mb : a_gmem.rows() / config::MLP_Mb;
    ...
    if (warpgroup::groupid() == config::NUM_CONSUMERS) {
        if (warpgroup::warpid() == 3 && warp::elect_leader()) {
            ...
            if constexpr (IS_WGRAD) {
                ...
                for (int idx = 0, k_block = k_start / MLP_Kb; idx < iters_per_task; ++idx, ++k_block) {
                    ...
                    if constexpr (IS_SHARED) { // BF16 AtB: A/B tiles are K-major slices of the normal-orientation activations
            ...
            int input_ring = 0;
            if constexpr (!IS_SHARED && IS_WGRAD) { // MXFP8 wgrad: both operands' scales follow the token-major layout
                const int macrobatch_k_offset = macrobatch_idx * (macrobatch_size / MLP_Kb);
                for (int idx = 0, k_block = k_start / MLP_Kb; idx < iters_per_task; ++idx, ++k_block) {
                    wait_for_wgrad_operands(idx, k_block * MLP_Kb);
                    wait(gemm_inputs_finished[input_ring], get_phasebit<1>(gemm_bitfield, input_ring));
        ...
        if (!IS_SHARED && d_fp8_gmem != nullptr) {
...
static __device__ __forceinline__ void dispatch_mlp_swiglu_combine_fwd_kernel(const globals_fwd &g) {
    ...
    if (cluster_idx < comm_clusters) {
        ...
        auto dispatch = [&](int macrobatch_idx, int task_idx) {
            dispatch_quantize_kernel(g.x_routed_send_buffer, g.x_fp8_routed, g.x_sc_routed, g.x_fp8_t_routed, g.x_sc_t_routed,
    ...
    for (int task_iter = 0; cluster_idx >= 0 && cluster_idx < true_num_clusters; ++task_iter) {
        ...
        if (compute_cluster_idx < shared_gate_up_tasks) {
            ...
            expert_grouped_gemm<true>(g.x_shared, g.w_shared_gate, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                                      ...
                                      gemm_inputs_arrived, gemm_scales_arrived, gemm_inputs_finished, gemm_outputs_arrived, gemm_outputs_finished, gemm_bitfield,
            ...
            expert_grouped_gemm<true>(g.x_shared, g.w_shared_up, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            swiglu_fwd<true>(g.gate_shared, g.up_shared, g.hidden_shared, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            expert_grouped_gemm<true>(g.hidden_shared, g.w_shared_down, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            if (minibatch_task_idx < minibatch_routed_gate_up_tasks) {
                ...
                expert_grouped_gemm<false>(g.x_fp8_routed, g.w_routed_gate, &g.x_sc_routed, &g.w_routed_gate_sc, nullptr, nullptr, nullptr, nullptr,
                                           g.y_routed, &g.gate_fp8_routed, &g.gate_sc_routed,
                ...
                expert_grouped_gemm<false>(g.x_fp8_routed, g.w_routed_up, &g.x_sc_routed, &g.w_routed_up_sc, nullptr, nullptr, nullptr, nullptr,
                                           g.y_routed, &g.up_fp8_routed, &g.up_sc_routed,
                                           g.tokens_per_expert, &g.x_routed_ready, nullptr, &g.gate_up_tile_ready, nullptr, nullptr,
                                           d_tt, a_sc_tt, b_sc_tt,
                                           gemm_inputs_arrived, gemm_scales_arrived, gemm_inputs_finished, gemm_outputs_arrived, gemm_outputs_finished, gemm_bitfield,
                                           num_tokens, macrobatch_size, g.minibatch_size, macrobatch_idx, minibatch_idx, task_idx, cta_rank,
                ...
                swiglu_fwd<false>(g.gate_fp8_routed, g.up_fp8_routed, g.hidden_fp8_routed,
                                  &g.gate_sc_routed, &g.up_sc_routed, &g.hidden_sc_routed, &g.hidden_fp8_t_routed, &g.hidden_sc_t_routed,
                                  g.gate_up_tile_ready, g.hidden_row_block_ready,
                                  swiglu_inputs_arrived, swiglu_bitfield,
                                  num_tokens, macrobatch_size, g.minibatch_size, macrobatch_idx, minibatch_idx, task_idx, cta_rank,
                ...
                expert_grouped_gemm<false>(g.hidden_fp8_routed, g.w_routed_down, &g.hidden_sc_routed, &g.w_routed_down_sc, nullptr, nullptr, nullptr, nullptr,
...
static __device__ __forceinline__ void dispatch_mlp_swiglu_combine_bwd_kernel(const globals_bwd &g) {
    ...
    if (cluster_idx < comm_clusters) {
        ...
        auto reverse_combine = [&](int macrobatch_idx, int task_idx) {
            dispatch_quantize_kernel(g.d_combine_buffer, g.d_y_fp8_routed, g.d_y_sc_routed, g.d_y_fp8_t_routed, g.d_y_sc_t_routed,
                                     g.schedule_peer_rank, g.schedule_peer_token_idx,
    ...
    for (int task_iter = 0; cluster_idx >= 0 && cluster_idx < true_num_clusters; ++task_iter) {
        ...
        if (compute_cluster_idx < shared_dgrad_down_tasks) {
            ...
            const int task_idx = compute_cluster_idx;
            expert_grouped_gemm<true>(g.d_y_shared, g.w_shared_down_T, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            expert_grouped_gemm<true>(g.d_gate_shared, g.w_shared_gate_T, nullptr, nullptr, &g.d_up_shared, &g.w_shared_up_T, nullptr, nullptr,
            ...
            expert_grouped_gemm<true, true>(g.d_y_shared, g.hidden_shared, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            expert_grouped_gemm<true, true>(g.d_gate_shared, g.x_shared, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            expert_grouped_gemm<true, true>(g.d_up_shared, g.x_shared, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            ...
            if (macrobatch_task_idx < num_replay_tasks) {
                ...
                if (minibatch_task_idx < minibatch_routed_gate_up_tasks) {
                    // Replay gate: gate_fp8_routed = quantize(x_fp8_routed @ w_routed_gate^T)
                    const int task_idx = minibatch_task_idx;
                    expert_grouped_gemm<false>(g.x_fp8_routed, g.w_routed_gate, &g.x_sc_routed, &g.w_routed_gate_sc, nullptr, nullptr, nullptr, nullptr,
                                               g.d_x_routed, &g.gate_fp8_routed, &g.gate_sc_routed,
                    ...
                    // Replay up: up_fp8_routed = quantize(x_fp8_routed @ w_routed_up^T)
                    const int task_idx = minibatch_task_idx - minibatch_routed_gate_up_tasks;
                    expert_grouped_gemm<false>(g.x_fp8_routed, g.w_routed_up, &g.x_sc_routed, &g.w_routed_up_sc, nullptr, nullptr, nullptr, nullptr,
                                               g.d_x_routed, &g.up_fp8_routed, &g.up_sc_routed,
                                               g.tokens_per_expert, &g.replayed_x_routed_ready, nullptr, &g.replayed_gate_up_ready, nullptr, nullptr,
                                               d_tt, a_sc_tt, b_sc_tt,
                                               gemm_inputs_arrived, gemm_scales_arrived, gemm_inputs_finished, gemm_outputs_arrived, gemm_outputs_finished, gemm_bitfield,
                                               num_tokens, macrobatch_size, g.minibatch_size, macrobatch_idx, minibatch_idx, task_idx, cta_rank,
                    ...
                    // Replay Swiglu: hidden_fp8_t_routed = quantize_t(silu(gate) * up)
                    const int task_idx = minibatch_task_idx - minibatch_routed_gate_up_tasks * 2;
                    swiglu_fwd<false>(g.gate_fp8_routed, g.up_fp8_routed, g.hidden_fp8_routed,
                                      &g.gate_sc_routed, &g.up_sc_routed, &g.hidden_sc_routed, &g.hidden_fp8_t_routed, &g.hidden_sc_t_routed,
                ...
                if (routed_task_idx < num_routed_tasks && minibatch_task_idx < minibatch_routed_dgrad_down_tasks) {
                    // Dgrad down: d_hidden_routed = dequant(d_y_fp8_routed) @ dequant(w_routed_down_T)^T
                    const int task_idx = minibatch_task_idx;
                    expert_grouped_gemm<false>(g.d_y_fp8_routed, g.w_routed_down_T, &g.d_y_sc_routed, &g.w_routed_down_T_sc, nullptr, nullptr, nullptr, nullptr,
                    ...
                    // Swiglu bwd: d_gate, d_up = quantize(swiglu_bwd(d_hidden_routed, dequant(gate), dequant(up))), normal + transposed
                    const int task_idx = minibatch_task_idx - minibatch_routed_dgrad_down_tasks;
                    swiglu_bwd<false>(g.d_hidden_routed, g.gate_fp8_routed, g.up_fp8_routed, g.d_gate_fp8_routed, g.d_up_fp8_routed,
                    ...
                    // Dgrad gate+up: d_x_routed = dequant(d_gate) @ dequant(w_routed_gate_T)^T + dequant(d_up) @ dequant(w_routed_up_T)^T
                    const int task_idx = minibatch_task_idx - minibatch_routed_dgrad_down_tasks - minibatch_routed_swiglu_bwd_tasks;
                    expert_grouped_gemm<false>(g.d_gate_fp8_routed, g.w_routed_gate_T, &g.d_gate_sc_routed, &g.w_routed_gate_T_sc,
                    ...
                    // Wgrad down: d_w_routed_down += dequant(d_y_fp8_t) @ dequant(hidden_fp8_t)^T (per expert)
                    const int task_idx = routed_task_idx - num_routed_tasks;
                    expert_grouped_gemm<false, true>(g.d_y_fp8_t_routed, g.hidden_fp8_t_routed, &g.d_y_sc_t_routed, &g.hidden_sc_t_routed, nullptr, nullptr, nullptr, nullptr,
                    ...
                    // Wgrad gate: d_w_routed_gate += dequant(d_gate_fp8_t) @ dequant(x_fp8_t)^T (per expert)
                    const int task_idx = routed_task_idx - num_routed_tasks - wgrad_matrix_tasks;
                    expert_grouped_gemm<false, true>(g.d_gate_fp8_t_routed, g.x_fp8_t_routed, &g.d_gate_sc_t_routed, &g.x_sc_t_routed, nullptr, nullptr, nullptr, nullptr,
                    ...
                    // Wgrad up: d_w_routed_up += dequant(d_up_fp8_t) @ dequant(x_fp8_t)^T (per expert)
                    const int task_idx = routed_task_idx - num_routed_tasks - 2 * wgrad_matrix_tasks;
                    expert_grouped_gemm<false, true>(g.d_up_fp8_t_routed, g.x_fp8_t_routed, &g.d_up_sc_t_routed, &g.x_sc_t_routed, nullptr, nullptr, nullptr, nullptr,
...
dispatch_mlp_swiglu_combine_bwd(
    ...
    // Pre-transposed weights (routed ones pre-quantized to MXFP8)
    ...
    const int schedule_capacity = schedule_peer_rank.size(0);
    const int hidden_dim = x.size(1);
    const int intermediate_dim = w_shared_gate_T.size(1);
    ...
    // Gradient tensors
    at::Tensor d_y_fp8_routed = at::empty({macrobatch_size, hidden_dim}, d_y_shared.options().dtype(at::kFloat8_e4m3fn));
    at::Tensor d_y_sc_routed = at::empty({macrobatch_size / 128, hidden_dim / 128, 32, 16}, d_y_shared.options().dtype(at::kByte));
    at::Tensor d_y_fp8_t_routed = at::empty({hidden_dim, macrobatch_size}, d_y_shared.options().dtype(at::kFloat8_e4m3fn));
    at::Tensor d_y_sc_t_routed = at::empty({hidden_dim / 128, macrobatch_size / 128, 32, 16}, d_y_shared.options().dtype(at::kByte));
    ...
    at::Tensor d_gate_fp8_routed = at::empty({macrobatch_size, intermediate_dim}, d_y_shared.options().dtype(at::kFloat8_e4m3fn));
    at::Tensor d_gate_sc_routed = at::empty({macrobatch_size / 128, intermediate_dim / 128, 32, 16}, d_y_shared.options().dtype(at::kByte));
    at::Tensor d_gate_fp8_t_routed = at::empty({intermediate_dim, macrobatch_size}, d_y_shared.options().dtype(at::kFloat8_e4m3fn));
    at::Tensor d_gate_sc_t_routed = at::empty({intermediate_dim / 128, macrobatch_size / 128, 32, 16}, d_y_shared.options().dtype(at::kByte));
    ...
    at::Tensor d_w_routed_gate = at::zeros({num_local_experts, intermediate_dim, hidden_dim}, d_y_shared.options());
    at::Tensor d_w_shared_up = at::zeros({intermediate_dim, hidden_dim}, d_y_shared.options());
    at::Tensor d_w_routed_up = at::zeros({num_local_experts, intermediate_dim, hidden_dim}, d_y_shared.options());
    ...
    globals_bwd g {
        ...
        .w_routed_up = kittens::py::tensor_to_gl<weight_fp8_gl>(w_routed_up),
        .w_routed_up_sc = kittens::py::tensor_to_gl<sc_gl>(w_routed_up_sc),
        .w_shared_gate_T = kittens::py::tensor_to_gl<weight_bf16_gl>(w_shared_gate_T),
        ...
        .w_shared_up_T = kittens::py::tensor_to_gl<weight_bf16_gl>(w_shared_up_T),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("dispatch_mlp_swiglu_combine_bwd", &dispatch_mlp_swiglu_combiner<4>::dispatch_mlp_swiglu_combine_bwd, "",
          ...
          pybind11::arg("w_shared_gate_T"), pybind11::arg("w_routed_gate_T"), pybind11::arg("w_routed_gate_T_sc"),
          pybind11::arg("w_shared_up_T"), pybind11::arg("w_routed_up_T"), pybind11::arg("w_routed_up_T_sc"),
          pybind11::arg("w_shared_down_T"), pybind11::arg("w_routed_down_T"), pybind11::arg("w_routed_down_T_sc"),
